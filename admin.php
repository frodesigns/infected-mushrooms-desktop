...
if ($_POST['itemname'])
{
	...
	$itemhistory = trim($_POST['itemhistory']);	
	$imagepath = trim($_POST['imagepath']);	

	mysql_select_db($db, $link);

	$sql = "INSERT INTO items (itemname, history) VALUES ('$itemname', '$itemhistory')";
	...
	if ($imagepath != "") {
		$sql2 = "UPDATE items SET imageurl = '$imagepath' WHERE itemname = '$itemname'";
		...
		&& ($_FILES["image"]["size"] < 50000))
			...
			if ($_FILES["image"]["error"] > 0)
				...
				$imageurl = "upload/" . $_FILES["image"]["name"];				
											
				mysql_select_db($db, $link);

				$sql2 = "UPDATE items SET imageurl = '$imageurl' WHERE itemname = '$itemname'";
...
if(!empty($_POST['u'])) {
	foreach ($_POST['u'] as $uid) {
		...
		$sql = "DELETE FROM readthreads WHERE id='$id'";
		if (!mysql_query($sql,$link))
		{
			die('Error: ' . mysql_error());
		}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="100%" valign="top" style="padding: 10px;"><h3 class="titlehdr">Administration Page</h3>
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="forms">
        <tr>
	  ...
	  ?></p>
      <table width="80%" border="0" align="center" cellpadding="10" cellspacing="0" class="forms" >
        <tr>
	  ...
	  // outputting the pages
		if ($total > $page_limit)
		{
		echo "<div class='pages'><strong>Pages:</strong> ";
		$i = 0;
		while ($i < $page_limit)
		{
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="forms">
          <tr> 
          ...
          <tr> 
			...
			<div style="display:none;font: normal 11px arial; padding:10px; background: #000" id="edit<?php echo $rrows['id']; ?>">
			
			<input type="hidden" name="id<?php echo $rrows['id']; ?>" id="id<?php echo $rrows['id']; ?>" value="<?php echo $rrows['id']; ?>">
			Display Name: <input name="full_name<?php echo $rrows['id']; ?>" id="full_name<?php echo $rrows['id']; ?>" type="text" size="10" value="<?php echo $rrows['full_name']; ?>" >
			...
			Can Vote?: <input class='votechange' id="user_vote<?php echo $rrows['id']; ?>" name="user_vote<?php echo $rrows['id']; ?>" type="checkbox" value="<?php echo $rrows['canvote']; ?>" <?php echo $checked; ?>>
			...
			onclick='$.get("do.php",{ cmd: "edit", pass:$("input#pass<?php echo $rrows['id']; ?>").val(),user_level:$("input#user_level<?php echo $rrows['id']; ?>").val(),user_vote:$("input#user_vote<?php echo $rrows['id']; ?>").val(),user_email:$("input#user_email<?php echo $rrows['id']; ?>").val(),user_name: $("input#user_name<?php echo $rrows['id']; ?>").val(),full_name: $("input#full_name<?php echo $rrows['id']; ?>").val(),id: $("input#id<?php echo $rrows['id']; ?>").val() } ,function(data){ $("#msg<?php echo $rrows['id']; ?>").html(data); });'> 
...
echo "<div class=\"msg\">User created with password $pwd....done.</div>"; 
	  ...
	  <h3 class="titlehdr">Add an Item</h3>
      <table width="80%" border="0" cellpadding="5" cellspacing="2" class="forms">
        <tr>
          <td><form name="form2" method="post" action="admin.php" enctype="multipart/form-data">
              <p>Item Name 
                <input name="itemname" maxlength="50" type="text" id="itemname">
			  </p>   
		      <p>Item History 
                <input name="itemhistory" maxlength="500" type="text" id="itemhistory" style="width: 80%;">
			  </p> 
			  <div class="images">
				...
				<?php
					...
					$images = glob("" . $directory . "{*.gif,*.png}", GLOB_BRACE);
        ...
        <tr>
          <td><form name="form1" method="post" action="admin.php">
              <p>User ID 
                ...
                (Type the username)</p>
			  <p>Character Name 
                <input name="full_name" maxlength="14" type="text" id="full_name">
              </p>
              <p>Email 
